...
private:
  ...
  constexpr uint8_t packTypes(Type type1, Type type2) {
    return ((static_cast<uint8_t>(type1) & Type1Mask) << Type1Shift) |
           ((static_cast<uint8_t>(type2) & Type2Mask) << Type2Shift);
  }
...
static Modifier getModifier(Type attackType, Type defendingType) {
  return (attackType == Type::NONE || defendingType == Type::NONE)
             ? Modifier::None
             : static_cast<Modifier>(pgm_read_byte(
                   &typeTable[static_cast<uint8_t>(attackType)]
                             [static_cast<uint8_t>(defendingType)]));
}

static uint16_t applyModifier(uint16_t baseValue, Type attackType,
                              DualType defendingType) {
